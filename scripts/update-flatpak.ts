...
await $`git clone git@github.com:flathub/io.github.sigmasd.recordapps.git recordapps`
  .cwd(tempDir.path);
Deno.chdir(tempDir.toString() + "/recordapps");

const newSha = await fetch(
  `https://github.com/sigmaSd/recordapps/archive/refs/tags/${version}.tar.gz`,
).then(async (res) => {
...
const oldSha = await fetch(
  `https://github.com/sigmaSd/recordapps/archive/refs/tags/${oldVersion}.tar.gz`,
).then(async (res) => {
  const hashBuffer = await crypto.subtle.digest("SHA-256", await res.bytes());
  const sha256 = Array.from(new Uint8Array(hashBuffer)).map((b) =>
    b.toString(16).padStart(2, "0")
...
await Deno.readTextFile("./io.github.sigmasd.recordapps.yml")
  .then((r) => r.replace(`${oldVersion}.tar.gz`, `${version}.tar.gz`))
  .then((r) => r.replace(oldSha, newSha))
  .then((r) => Deno.writeTextFile("./io.github.sigmasd.recordapps.yml", r));
